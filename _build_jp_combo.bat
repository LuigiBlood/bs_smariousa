@echo off
echo Assembling Combo Version... (Japanese Version)
"./tools/conv/superfamiconv.exe" -i "./gfx/jp/bg1_jp.png" -p "./gfx/temp/bg1_jp.pal" -t "./gfx/temp/bg1_jp.chr" -m "./gfx/temp/bg1_jp.map" -P 7
"./tools/conv/superfamiconv.exe" -i "./gfx/bg2.png" -p "./gfx/temp/bg2.pal" -t "./gfx/temp/bg2.chr" -m "./gfx/temp/bg2.map"
"./tools/conv/superfamiconv.exe" -i "./gfx/icons.png" -p "./gfx/temp/icons.pal" -t "./gfx/temp/icons.chr" -S -W 16 -H 16
"./tools/conv/superfamiconv.exe" -i "./gfx/jp/obj_logo_jp.png" -p "./gfx/temp/obj_logo_jp.pal" -t "./gfx/temp/obj_logo_jp.chr" -S -W 16 -H 16
"./tools/bass/bass" build_jp_combo.asm
echo Done.
pause